...
class Textman{
    ...
    public static void encode(){
        ...
        Node root = q.peek(); //root of textman tree 
        while(q.size() > 1){  // creating textman tree 
            Node left = q.peek();
            q.poll();
            Node right = q.peek();
            q.poll();
        ...
        try {   
            f = new File(file+".Textman");
            FileOutputStream fos = new FileOutputStream(file+".Textman") ;  //Storing b[] in File.textman
                fos.write(b);
                fos.close();
            }catch (IOException e) {
                System.out.println("An error occurred.");
    ...
    public static void decode(){
        ...
        String text="";
        try{
            FileInputStream is =new FileInputStream(file+".Textman");
        ...
        try{                                 //reading File.table and recreating Textman Tree
       ...
       Node t=root ;
       for(i=0 ; i< code.length(); i++){    //decoding using textman tree
            if(t== null)
                break;
